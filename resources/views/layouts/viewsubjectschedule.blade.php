@extends('layouts.viewsubjects')
@section('SubjectSchedule')
					<style> 
...
@include('layouts.include.functions')
						...
						$subjectschedule = Time_table::join('course_details','time_tables.course_code','=','course_details.id')->where('course_details.offered_by', $username)->where('course_details.course_code', $subject)->where('time_tables.department_code',$department)->select('time_tables.*')->orderby('time_tables.stime','ASC')->get();
		...
		<table style='width:95%' border='1'>
		  <tr>
			<td align='center'><span style='font-weight:bold'>{{ "Subject: ".$subject }}</span></td>
			<?php
				$slot=0;
				$time=7;
		...
		<table border='1'>
			...
			<?php
					...
					if(str_replace(" ", "",$fullname) == null)
							$fullname = $username;
					$course_title = Course::where('course_code',$subject)->first()->course_title;
					echo "<tr>";
					echo "<td>".$subject."</td>";
